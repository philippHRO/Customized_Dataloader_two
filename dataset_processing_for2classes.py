...
class DatasetProcessing(Dataset):
    ...
    def __init__(self, data_path, img_path, search_classes, transform=None):
        ...
        for name in self.the_filename_list:
            if search_classes[0] in name:
                the_target_list.append([1, 0])
            elif search_classes[1] in name:
                the_target_list.append([0, 1])
            else:
                print("Something went wrong with the target creation!")
        #self.label = np.asarray(the_target_list, dtype=target_dtype) #np.int64
        self.label = torch.from_numpy(np.asarray(the_target_list, dtype=np.int16)).float()
    ...
    def __getitem__(self, index):
        ...
        #label = torch.from_numpy(self.label[index]) # original
